...
def _get_language_order():
    territory_info_file = (
        '../raw_data/cldr_core/supplemental/territoryInfo.json'
    )
    ...
    for territory in territory_info_data:
        ...
        try:
            ...
            for language in lang_dict:
                language_population = (
                    float(lang_dict[language]['_populationPercent'])
                    * population
                )
    ...
    supplementary_languages = [
        x[:-5] for x in os.listdir(supplementary_date_directory)
    ]
    available_languages = set(cldr_languages).union(
        set(supplementary_languages)
    )
    language_order = [
        shortname
        for shortname in language_order
        if shortname in available_languages
    ]
    ...
    for language in absent_languages:
        ...
        if parent_language in language_order:
            language_order.insert(
                language_order.index(parent_language) + 1, language
            )
        else:
...
def main():
    ...
    for key in language_order:
        if key in language_locale_dict.keys():
            complete_language_locale_dict[key] = sorted(
                language_locale_dict[key]
            )
        else:
