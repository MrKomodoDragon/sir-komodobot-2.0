...
def get_raw_data():
    ...
    if not os.path.isdir(raw_data_directory):
        ...
        Repo.clone_from(
            cldr_dates_full_url, cldr_dates_full_dir, branch='master'
        )
...
def get_dict_difference(parent_dict, child_dict):
    ...
    for key, child_value in child_dict.items():
        ...
        if not parent_value:
            ...
            child_specific_value = get_dict_difference(
...
def combine_dicts(primary_dict, supplementary_dict):
    ...
    for key, value in primary_dict.items():
        if key in supplementary_dict:
            if isinstance(value, list):
                ...
                combined_dict[key] = combine_dicts(
                    value, supplementary_dict[key]
                )
            else:
    ...
    remaining_keys = [
        key
        for key in supplementary_dict.keys()
