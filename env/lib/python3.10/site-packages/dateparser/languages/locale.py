...
class Locale:
    ...
    def is_applicable(self, date_string, strip_timezone=False, settings=None):
        ...
        """
        if strip_timezone:
            date_string, _ = pop_tz_offset_from_string(
    ...
    def count_applicability(self, text, strip_timezone=False, settings=None):
        ...
        for sent in sentences:
            tokens.extend(
                self._split(sent, keep_formatting=False, settings=settings)
            )
    ...
    def _get_relative_translations(self, settings=None):
        if settings.NORMALIZE:
            ...
            if self._relative_translations is None:
                self._relative_translations = (
                    self._generate_relative_translations(normalize=False)
                )
    ...
    def _split_tokens_by_known_words(
        self, tokens, keep_formatting, settings=None
    ):
        dictionary = self._get_dictionary(settings)
        for i, token in enumerate(tokens):
    ...
    def _get_simplifications(self, settings=None):
        ...
        if settings.NORMALIZE:
            if self._normalized_simplifications is None:
                ...
                simplifications = self._generate_simplifications(
                    normalize=True
                )
                for simplification in simplifications:
                    pattern, replacement = list(simplification.items())[0]
                    ...
                    self._normalized_simplifications.append(
                        {pattern: replacement}
                    )
    ...
    def _generate_simplifications(self, normalize=False):
        ...
        for simplification in self.info.get('simplifications', []):
            ...
            if isinstance(value, int):
                ...
                c_simplification[key] = (
                    normalize_unicode(value) if normalize else value
                )
    ...
    def _join(self, tokens, separator=' ', settings=None):
        ...
        for i in range(1, len(tokens)):
            ...
            if (
                left not in capturing_splitters
                and right not in capturing_splitters
            ):
                joined += separator
            joined += right
    ...
    def _generate_normalized_dictionary(self, settings=None):
        self._normalized_dictionary = NormalizedDictionary(
