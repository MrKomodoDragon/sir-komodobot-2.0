    ...
    def _validate_pertain_list(cls, language_id, info):
        ...
        if isinstance(pertain_tokens_list, list):
            ...
            cls.get_logger().error(
                ...
                {
                    'id': language_id,
                    'type': type(pertain_tokens_list).__name__,
                },
            )
            result = False
    ...
    def _validate_simplifications(cls, language_id, info):
        ...
        if isinstance(simplifications_list, list):
            for simplification in simplifications_list:
                ...
                not_used_groups -= {
                    0
                }  # Entire substring is not required to be used
                ...
                if not_used_groups:
                    cls.get_logger().error(
                        ...
                        'groups %(groups)s were not used',
                        {
                            ...
                            'id': language_id,
                            'groups': ', '.join(
                                map(str, sorted(not_used_groups))
                            ),
                        },
                    )
                    result = False
            ...
            cls.get_logger().error(
                ...
                {
                    'id': language_id,
                    'type': type(simplifications_list).__name__,
                },
            )
            result = False
