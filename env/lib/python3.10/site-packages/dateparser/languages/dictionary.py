...
class Dictionary:
    ...
    def __init__(self, locale_info, settings=None):
        ...
        for word in KNOWN_WORD_TOKENS:
            if word in locale_info:
                ...
                dictionary.update(
                    zip_longest(translations, [], fillvalue=word)
                )
        dictionary.update(zip_longest(ALWAYS_KEEP_TOKENS, ALWAYS_KEEP_TOKENS))
        dictionary.update(
            zip_longest(
                map(methodcaller('lower'), PARSER_KNOWN_TOKENS),
                PARSER_KNOWN_TOKENS,
            )
        )
        ...
        for key, value in relative_type.items():
            ...
            dictionary.update(
                zip_longest(relative_translations, [], fillvalue=key)
            )
        ...
        self._relative_strings = list(
            chain.from_iterable(relative_type_regex.values())
        )
    ...
    def _split_by_known_words(self, string, keep_formatting):
        ...
        if unparsed and self._should_capture(unparsed, keep_formatting):
            splitted = (
                self._split_by_numerals(unparsed, keep_formatting) + splitted
            )
        if unknown:
            splitted.extend(
                self._split_by_known_words(unknown, keep_formatting)
            )
    ...
    def _get_sorted_words_from_cache(self):
        if (
            ...
            )[self.info['name']] = sorted(
                [key for key in self], key=len, reverse=True
            )
    ...
    def _construct_split_regex(self):
        ...
        self._split_regex_cache.setdefault(self._settings.registry_key, {})[
            self.info['name']
    ...
    def _get_sorted_relative_strings_from_cache(self):
        if (
            self._settings.registry_key
            not in self._sorted_relative_strings_cache
            or self.info['name']
            not in self._sorted_relative_strings_cache[
                self._settings.registry_key
            ]
        ...
        return self._sorted_relative_strings_cache[
            self._settings.registry_key
        ][self.info['name']]
    ...
    def _get_split_relative_regex_cache(self):
        if (
            ...
            or self.info['name']
            not in self._split_relative_regex_cache[
                self._settings.registry_key
            ]
        ):
    ...
    def _get_match_relative_regex_cache(self):
        if (
            ...
            or self.info['name']
            not in self._match_relative_regex_cache[
                self._settings.registry_key
            ]
        ):
...
class NormalizedDictionary(Dictionary):
    ...
    def _normalize(self):
        ...
        for key in conflicting_keys:
            normalized = normalize_unicode(key)
            if key in (
                self.info.get('skip', []) + self.info.get('pertain', [])
            ):
                new_dict[normalized] = self._dictionary[key]
        self._dictionary = new_dict
        self._relative_strings = list(
            map(normalize_unicode, self._relative_strings)
        )
