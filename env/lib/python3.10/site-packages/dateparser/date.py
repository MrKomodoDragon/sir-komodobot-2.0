...
def date_range(begin, end, **kwargs):
    ...
    if kwargs.get('months', 0) > 0 and (date.year, date.month) == (
        end.year,
        end.month,
    ):
        yield end
...
class _DateLocaleParser:
    def __init__(self, locale, date_string, date_formats, settings=None):
        ...
        if not (
            date_formats is None
            or isinstance(date_formats, (list, tuple, Set))
        ):
            raise TypeError(
    ...
    def _is_valid_date_data(self, date_data):
        ...
        if not date_data['date_obj'] or not date_data['period']:
            return False
        if date_data['date_obj'] and not isinstance(
            date_data['date_obj'], datetime
        ):
            return False
    ...
    def __repr__(self):
        if sys.version_info < (3, 6):  # python 3.5 compatibility
            properties_text = 'date_obj={}, period={}, locale={}'.format(
                self.date_obj.__repr__(),
                self.period.__repr__(),
                self.locale.__repr__(),
            )
        else:
            properties_text = ', '.join(
                '{}={}'.format(prop, val.__repr__())
...
class DateDataParser:
    ...
    def __init__(
        ...
        if region is not None and not isinstance(region, str):
            raise TypeError(
                'region argument must be str (%r given)' % type(region)
            )
        ...
        if not locales and use_given_order:
            raise ValueError(
                'locales must be given if use_given_order is True'
            )
    ...
    def get_date_data(self, date_string, date_formats=None):
        ...
        *Period* represents the granularity of date parsed from the given string.
        ...
        res = parse_with_formats(
            date_string, date_formats or [], self._settings
        )
        if res['date_obj']:
            return res
