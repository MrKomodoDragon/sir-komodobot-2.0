...
class Settings:
    ...
    * `PARSERS`
    ...
    def get_key(cls, settings=None):
        ...
        keys = sorted(
            ['%s-%s' % (key, str(settings[key])) for key in settings]
        )
...
def _check_repeated_values(setting_name, setting_value):
    if len(setting_value) != len(set(setting_value)):
        raise SettingValidationError(
            'There are repeated values in the "{}" setting'.format(
                setting_name
            )
        )
    return
...
def check_settings(settings):
    ...
    for setting in modified_settings:
        if setting not in settings_values:
            raise SettingValidationError(
                '"{}" is not a valid setting'.format(setting)
            )
    ...
    for setting_name, setting_value in modified_settings.items():
        ...
        # check values:
        if (
            setting_props.get('values')
            and setting_value not in setting_props['values']
        ):
            raise SettingValidationError(
