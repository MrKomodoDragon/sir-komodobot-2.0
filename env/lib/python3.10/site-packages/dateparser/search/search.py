...
class _ExactLanguageSearch:
    ...
    def choose_best_split(
        self, possible_parsed_splits, possible_substrings_splits
    ):
        rating = []
        for i in range(len(possible_parsed_splits)):
            ...
            for j, item in enumerate(possible_parsed_splits[i]):
                if item[0]['date_obj'] is None:
                    not_parsed += 1
                if not any(
                    char.isdigit() for char in possible_substrings_splits[i][j]
                ):
            ...
            rating.append(
                [
                    ...
                    if num_substrings_without_digits == 0
                        ...
                        float(num_substrings_without_digits)
                        / float(num_substrings)
                    ),
                ]
            )
        ...
        if item.count(splitter) <= 2:
            ...
            for i in range(2, 4):
                ...
                for j in range(0, len(item_all_split), i):
                    ...
                    original_join = splitter.join(
                        original_all_split[j : j + i]
                    )
                    item_partially_split.append(item_join)
                    original_partially_split.append(original_join)
                all_possible_splits.append(
                    [item_partially_split, original_partially_split]
                )
    ...
    def split_if_not_parsed(self, item, original):
        ...
        for splitter in splitters:
            if splitter in item and item.count(splitter) == original.count(
                splitter
            ):
    ...
    def parse_item(
        self, parser, item, translated_item, parsed, need_relative_base
    ):
                            ...
                            if split_translated:
                                for j, jtem in enumerate(split_translated):
                                    if len(jtem) > 2:
                                        ...
                                        current_parsed.append(
                                            (parsed_jtem, is_relative_jtem)
                                        )
                                        current_substrings.append(
                                            split_original[j].strip(
                                                ' .,:()[]-'
                                            )
                                        )
                            else:
...
class DateSearchWithDetection:
    ...
    def detect_language(self, text, languages):
        if isinstance(languages, (list, tuple, Set)):
            ...
            raise TypeError(
                'languages argument must be a list (%r given)'
                % type(languages)
            )
