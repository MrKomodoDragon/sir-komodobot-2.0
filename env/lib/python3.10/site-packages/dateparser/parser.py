...
class _time_parser:
    ...
    def __call__(self, timestring):
        ...
        for directive in self.time_directives:
            ...
            raise ValueError(
                '%s does not seem to be a valid time string' % _timestring
            )
...
class _no_spaces_parser:
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.date_formats = {
            ...
            '%m%y%d': sorted(
                self._all,
                key=lambda x: x.lower().startswith('%m%y%d'),
                reverse=True,
            ),
            '%y%m%d': sorted(
                self._all,
                key=lambda x: x.lower().startswith('%y%m%d'),
                reverse=True,
            ),
            '%y%d%m': sorted(
                self._all,
                key=lambda x: x.lower().startswith('%y%d%m'),
                reverse=True,
            ),
            '%d%m%y': sorted(
                self._all,
                key=lambda x: x.lower().startswith('%d%m%y'),
                reverse=True,
            ),
            '%d%y%m': sorted(
                self._all,
                key=lambda x: x.lower().startswith('%d%y%m'),
                reverse=True,
            ),
        }
...
class _parser:
    ...
    def __init__(self, tokens, settings):
        ...
        for index, token_type_original_index in enumerate(
            self.filtered_tokens
        ):
            ...
            if self.time is None:
                ...
                try:
                    ...
                    _is_before_period = (
                        self.tokens[original_index + 1][0] == '.'
                    )
                    _is_after_period = (
                        original_index != 0
                    ...
                    if _is_before_period and not _is_after_period:
                        ...
                        if (
                            next_token_is_last
                            or self.tokens[index_in_tokens_for_next_token + 1][
                                0
                            ]
                            != '.'
                        ):
    ...
    def _get_correct_leap_year(self, prefer_dates_from, current_year):
        ...
        return (
            next_leap_year if next_leap_year_is_closer else previous_leap_year
        )
    ...
    def _parse(self, type, token, skip_component=None):
        def set_and_return(
            token, type, component, dateobj, skip_date_order=False
        ):
