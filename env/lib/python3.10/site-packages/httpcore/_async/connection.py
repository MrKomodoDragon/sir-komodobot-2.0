...
class AsyncHTTPConnection(AsyncHTTPTransport):
    def __init__(
        ...
        self.connection: Union[
            None, AsyncHTTP11Connection, AsyncHTTP2Connection
        ] = None
    ...
    async def request(
        ...
        async with self.request_lock:
            if self.state == ConnectionState.PENDING:
                if not self.socket:
                    logger.trace(
                        'open_socket origin=%r timeout=%r',
                        self.origin,
                        timeout,
                    )
        ...
        logger.trace(
            'connection.request method=%r url=%r headers=%r',
            method,
            url,
            headers,
        )
    ...
    def is_connection_dropped(self) -> bool:
        return (
            ...
            and self.connection.is_connection_dropped()
        )
    ...
    async def start_tls(
        ...
        if self.connection is not None:
            ...
            logger.trace(
                'start_tls complete hostname=%r timeout=%r', hostname, timeout
            )
