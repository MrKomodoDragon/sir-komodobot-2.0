...
class AsyncHTTP2Connection(AsyncHTTPTransport):
    ...
    async def request(
        ...
        try:
            ...
            return await h2_stream.request(
                method, url, headers, stream, timeout
            )
        except Exception:
    ...
    async def receive_events(self, timeout: TimeoutDict) -> None:
        ...
        for event in events:
            ...
            logger.trace(
                'receive_event stream_id=%r event=%s', event_stream_id, event
            )
    ...
    async def send_headers(
        ...
        logger.trace(
            'send_headers stream_id=%r headers=%r', stream_id, headers
        )
...
class AsyncHTTP2Stream:
    def __init__(
        self, stream_id: int, connection: AsyncHTTP2Connection
    ) -> None:
    ...
    async def request(
        ...
        headers = (
            [] if headers is None else [(k.lower(), v) for (k, v) in headers]
        )
        ...
        has_body = (
            b'content-length' in seen_headers
            or b'transfer-encoding' in seen_headers
        )
        ...
        stream = AsyncByteStream(
            aiterator=self.body_iter(timeout),
    ...
    async def send_headers(
        ...
        await self.connection.send_headers(
            self.stream_id, headers, end_stream, timeout
        )
    ...
    async def receive_response(
        ...
        """
        while True:
            event = await self.connection.wait_for_event(
                self.stream_id, timeout
            )
