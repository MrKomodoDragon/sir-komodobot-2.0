...
class SyncConnectionPool(SyncHTTPTransport):
    ...
    * **http2** - `bool` - Enable HTTP/2 support.
    ...
    def __init__(
        ...
        self._ssl_context = (
            SSLContext() if ssl_context is None else ssl_context
        )
    ...
    def _connection_acquiry_lock(self) -> SyncLock:
        if not hasattr(self, '_internal_connection_acquiry_lock'):
            self._internal_connection_acquiry_lock = (
                self._backend.create_lock()
            )
    ...
    def _get_connection_from_pool(
        ...
        if reuse_connection is not None:
            ...
            self._http2 and pending_connection is not None and not seen_http11
        ):
    ...
    def _connections_for_origin(
        self, origin: Origin
    ) -> Set[SyncHTTPConnection]:
