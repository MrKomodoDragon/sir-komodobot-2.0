...
class H2Stream(object):
    ...
    def __init__(
        self, stream_id, config, inbound_window_size, outbound_window_size
    ):
    ...
    def send_headers(self, headers, encoder, end_stream=False):
        ...
        if (not self.state_machine.client) and is_informational_response(
            headers
        ):
            if end_stream:
                raise ProtocolError(
    ...
    def _build_hdr_validation_flags(self, events):
        ...
        is_response_header = isinstance(
            events[0],
            (_ResponseSent, ResponseReceived, InformationalResponseReceived),
        )
