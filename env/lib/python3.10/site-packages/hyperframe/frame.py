...
class Frame(object):
    ...
    def __init__(self, stream_id, flags=()):
        ...
        if (
            not self.stream_id
            and self.stream_association == _STREAM_ASSOC_HAS_STREAM
        ):
            raise ValueError('Stream ID must be non-zero')
        if (
            self.stream_id
            and self.stream_association == _STREAM_ASSOC_NO_STREAM
        ):
    ...
    def __repr__(self):
        ...
        if len(body) > 20:
            body = body[:20] + '...'
        return ('{type}(Stream: {stream}; Flags: {flags}): {body}').format(
...
class DataFrame(Padding, Frame):
    ...
    def parse_body(self, data):
        ...
        self.data = data[
            padding_data_length : len(data) - self.total_padding
        ].tobytes()
...
class HeadersFrame(Padding, Priority, Frame):
    ...
    def parse_body(self, data):
        ...
        self.body_len = len(data)
        self.data = data[
            priority_data_length : len(data) - self.total_padding
        ].tobytes()
